...
public class Challenge {
    ...
    public static void main(String[] args) {
        ...
        // Answer 1 goes here
        List<String> peopleName = people.stream().map(element -> element.name).collect(Collectors.toList());
        ...
        // Answer 2 goes here
        List<Car> blueCars = cars.stream().filter(element -> element.color.equals("blue")).collect(Collectors.toList());
        ...
        BinaryOperator<Float> op = (acc, x) -> {
            return acc + x;
        };

        float sum = employees.stream().map(e -> e.salary).reduce(0f, op);
